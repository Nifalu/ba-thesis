...
*Timings and time-based attacks:* In a real system, messages are not stored on the bus. If necessary, individual components have buffers to store incoming messages until they are processed. Sophisticated priority mechanisms such as *CSMA/CD+AMP* are usually used to avoid collisions and ensure that high-priority messages are processed first. The _MCS Analyser_ does not support such mechanisms and therefore does not detect anything related to message timings or priorities.
